...
__global__ void update_repeat_times_v2(const int64_t *pre_ids,
    ...
    const int seq_len_now = seq_lens_encoder[bi];
    int tid = threadIdx.x;
    const int64_t *input_ids_now = input_ids + bi * max_input_length;
    const int64_t *pre_ids_now = pre_ids + bi * length_id;
    int *repeat_times_now = repeat_times + bi * length;
    int length_id_int32 = static_cast<int>(length_id);
    const int loop_len = seq_len_now > length_id_int32 ? seq_len_now : length_id_int32;
    for (int i = tid; i < loop_len; i += blockDim.x) {
        if (i < length_id_int32) {
            int64_t id = pre_ids_now[i];
            if (id >= 0) {
                atomicAdd(&repeat_times_now[id], 1);
            }
        }
        if (i > 0 && i < seq_len_now) {
    ...
    if (tid == 0) {
        atomicAdd(&repeat_times_now[first_token_ids[bi]], 1);
    }
}
...
void token_penalty_multi_scores_kernel_v2(const paddle::Tensor& input_ids,
    ...
    int64_t max_input_length = input_ids.shape()[1];

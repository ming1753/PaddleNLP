...
def fused_moe_kernel_zkk(
    ...
    # if MUL_ROUTED_WEIGHT and False:
    #     c_ptrs = c_ptr + stride_cm * (offs_token[:, None] // 8) + stride_cn * offs_cn[None, :]
...
def moe_align_block_size_stage3_api(place_holder, num_tokens_post_pad, tokens_cnts, cumsum, num_experts=-1, block_size=64):
    ...
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op(op_name, place_holder, num_tokens_post_pad, tokens_cnts, cumsum, num_experts, block_size)
        return outs[0]
    else:
        helper = LayerHelper(op_name, **locals())
        inputs = {
...
def moe_align_block_size_stage4_api(place_holder,
                                    topk_ids, 
                                    sorted_token_ids, 
                                    expert_ids, 
                                    tokens_cnts, 
...
def moe_align_block_size_triton(
    ...
    useless = moe_align_block_size_stage3_api(useless, num_tokens_post_pad, tokens_cnts, cumsum, num_experts, block_size)
    useless = moe_align_block_size_stage4_api(useless, topk_ids, sorted_token_ids, expert_ids, tokens_cnts, cumsum, num_experts, block_size)
    return
